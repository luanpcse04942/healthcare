...
public class UserDAO extends DBContext {
    ...
    public ArrayList<Account> searchDoctor(String codeSearch,int start, int total) {
        String sql = "SELECT * FROM [User] a join (select u.userId ,CONCAT(u.[firstName],' ',u.[lastName]) as name from [dbo].[User] u) t on t.userId = a.userId where t.name like '%" + codeSearch + "%' and a.roleId like 2 ORDER BY a.userId OFFSET " + start + " ROWS FETCH NEXT " + total + " ROWS ONLY";
        PreparedStatement ps = null;
        ResultSet rs = null;
        DBContext db = new DBContext();
        ArrayList<Account> listAccount = new ArrayList<>();
